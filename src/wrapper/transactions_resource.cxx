  ...
  ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(options), key, value)
  {
    ...
    if (zend_binary_strcmp(ZSTR_VAL(key), ZSTR_LEN(key), ZEND_STRL("queryOptions")) == 0) {
      ...
      ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(value), k, v)
      {
        if (zend_binary_strcmp(ZSTR_VAL(k), ZSTR_LEN(k), ZEND_STRL("scanConsistency")) == 0) {
          ...
          } else if (zend_binary_strcmp(Z_STRVAL_P(v), Z_STRLEN_P(v), ZEND_STRL("requestPlus")) ==
                     0) {
            config.query_config().scan_consistency(query_scan_consistency::request_plus);
          } else {
            return { errc::common::invalid_argument,
                     ERROR_LOCATION,
                     fmt::format("unknown scanConsistency: {}",
                                 std::string_view(Z_STRVAL_P(v), Z_STRLEN_P(v))) };
          }
        }
      }
    ...
    if (zend_binary_strcmp(ZSTR_VAL(key), ZSTR_LEN(key), ZEND_STRL("cleanupOptions")) == 0) {
      ...
      ZEND_HASH_FOREACH_STR_KEY_VAL(Z_ARRVAL_P(value), k, v)
      {
        ...
        ASSIGN_NEGATED_BOOLEAN_OPTION(
          "disableLostAttemptCleanup", config.cleanup_config().cleanup_lost_attempts, k, v);
        ASSIGN_NEGATED_BOOLEAN_OPTION(
          "disableClientAttemptCleanup", config.cleanup_config().cleanup_client_attempts, k, v);
      }
      ZEND_HASH_FOREACH_END();
    }
  ...
  -> std::pair<zend_resource*, core_error_info>
{
  ...
  if (auto metadata_collection = config.metadata_collection();
      metadata_collection && !metadata_collection->bucket.empty()) {
