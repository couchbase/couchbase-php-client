...
public:
  ...
  COUCHBASE_API
  core_error_info analytics_query(zval* return_value,
  ...
  COUCHBASE_API
  core_error_info search_query(zval* return_value,
                               const zend_string* index_name,
                               const zend_string* query,
                               const zval* options);
  ...
  COUCHBASE_API
  core_error_info diagnostics(zval* return_value,
                              const zend_string* report_id,
                              const zval* options);

  COUCHBASE_API
  core_error_info search_index_get(zval* return_value,
                                   const zend_string* index_name,
                                   const zval* options);
  ...
  COUCHBASE_API
  core_error_info search_index_drop(zval* return_value,
                                    const zend_string* index_name,
                                    const zval* options);

  COUCHBASE_API
  core_error_info search_index_get_documents_count(zval* return_value,
                                                   const zend_string* index_name,
                                                   const zval* options);

  COUCHBASE_API
  core_error_info search_index_control_ingest(zval* return_value,
                                              const zend_string* index_name,
                                              bool pause,
                                              const zval* options);

  COUCHBASE_API
  core_error_info search_index_control_query(zval* return_value,
                                             const zend_string* index_name,
                                             bool allow,
                                             const zval* options);

  COUCHBASE_API
  core_error_info search_index_control_plan_freeze(zval* return_value,
                                                   const zend_string* index_name,
                                                   bool freeze,
                                                   const zval* options);

  COUCHBASE_API
  core_error_info search_index_analyze_document(zval* return_value,
                                                const zend_string* index_name,
                                                const zend_string* document,
                                                const zval* options);

  COUCHBASE_API
  core_error_info scope_search_index_get(zval* return_value,
                                         const zend_string* bucket_name,
  ...
  COUCHBASE_API
  core_error_info scope_search_index_get_all(zval* return_value,
                                             const zend_string* bucket_name,
                                             const zend_string* scope_name,
                                             const zval* options);

  COUCHBASE_API
  core_error_info scope_search_index_upsert(zval* return_value,
                                            const zend_string* bucket_name,
                                            const zend_string* scope_name,
                                            const zval* index,
                                            const zval* options);
  ...
  COUCHBASE_API
  core_error_info scope_search_index_drop(zval* return_value,
                                          const zend_string* bucket_name,
                                          const zend_string* scope_name,
                                          const zend_string* index_name,
                                          const zval* options);
  ...
  COUCHBASE_API
  core_error_info scope_search_index_get_documents_count(zval* return_value,
                                                         const zend_string* bucket_name,
                                                         const zend_string* scope_name,
                                                         const zend_string* index_name,
                                                         const zval* options);
  ...
  COUCHBASE_API
  core_error_info scope_search_index_control_ingest(zval* return_value,
                                                    const zend_string* bucket_name,
                                                    const zend_string* scope_name,
                                                    const zend_string* index_name,
                                                    bool pause,
  ...
  COUCHBASE_API
  core_error_info scope_search_index_control_query(zval* return_value,
                                                   const zend_string* bucket_name,
                                                   const zend_string* scope_name,
                                                   const zend_string* index_name,
                                                   bool allow,
  ...
  COUCHBASE_API
  core_error_info scope_search_index_control_plan_freeze(zval* return_value,
                                                         const zend_string* bucket_name,
                                                         const zend_string* scope_name,
                                                         const zend_string* index_name,
                                                         bool freeze,
  ...
  COUCHBASE_API
  core_error_info scope_search_index_analyze_document(zval* return_value,
                                                      const zend_string* bucket_name,
                                                      const zend_string* scope_name,
                                                      const zend_string* index_name,
                                                      const zend_string* document,
  ...
  COUCHBASE_API
  core_error_info bucket_create(zval* return_value,
                                const zval* bucket_settings,
                                const zval* options);

  COUCHBASE_API
  core_error_info bucket_update(zval* return_value,
                                const zval* bucket_settings,
                                const zval* options);
  ...
  COUCHBASE_API
  core_error_info scope_create(zval* return_value,
                               const zend_string* bucket_name,
                               const zend_string* scope_name,
                               const zval* options);

  COUCHBASE_API
  core_error_info scope_drop(zval* return_value,
                             const zend_string* bucket_name,
                             const zend_string* scope_name,
                             const zval* options);

  COUCHBASE_API
  core_error_info collection_create(zval* return_value,
                                    const zend_string* bucket_name,
                                    const zend_string* scope_name,
  ...
  COUCHBASE_API
  core_error_info collection_drop(zval* return_value,
                                  const zend_string* bucket_name,
                                  const zend_string* scope_name,
                                  const zend_string* collection_name,
                                  const zval* options);
  ...
  COUCHBASE_API
  core_error_info collection_update(zval* return_value,
                                    const zend_string* bucket_name,
                                    const zend_string* scope_name,
                                    const zend_string* collection_name,
                                    const zval* settings,
  ...
  COUCHBASE_API
  core_error_info change_password(zval* return_value,
  ...
  COUCHBASE_API
  core_error_info query_index_get_all(zval* return_value,
                                      const zend_string* bucket_name,
                                      const zval* options);
  ...
  COUCHBASE_API
  core_error_info query_index_drop(const zend_string* bucket_name,
                                   const zend_string* index_name,
                                   const zval* options);
  ...
  COUCHBASE_API
  core_error_info query_index_build_deferred(zval* return_value,
                                             const zend_string* bucket_name,
                                             const zval* options);
